# terraform-azurerm-caf-windows_virtual_machineV2

Deploys an Azure Windows Virtual Machine with associated NICs, managed disks, backup, auto-shutdown, and Key Vault password management following the SSC CAF naming standard.

<!-- DO NOT EDIT BELOW — maintained by hand above and terraform-docs below -->
...
| automatic_updates_enabled                              | true,false                               | No       | true                |
...
| bypass_platform_safety_checks_on_user_schedule_enabled | true,false                               | No       | true                |
...
## Migration notes (azurerm 4.x)

### `vm_agent_platform_updates_enabled` removed

This field is **read-only** in azurerm 4.x and can no longer be set by Terraform. If you previously had:
...
### `backup_policy` for jump servers

When `jump_server = true` the module skips the Recovery Services Vault data source lookup. In this path `backup_policy` is passed **directly** as the `backup_policy_id` — it **must** be a full ARM resource ID:

```
/subscriptions/<sub-id>/resourceGroups/<rg>/providers/Microsoft.RecoveryServices/vaults/<rsv>/backupPolicies/<policy>
```
