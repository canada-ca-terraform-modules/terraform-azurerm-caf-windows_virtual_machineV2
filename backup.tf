...
resource "azurerm_backup_protected_vm" "backup_vm" {
  ...
  backup_policy_id    = try(var.windows_VM.jump_server, false) == true ? try(var.windows_VM.backup_policy, local.backup-policy-name) : data.azurerm_backup_policy_vm.backup_policy[0].id
