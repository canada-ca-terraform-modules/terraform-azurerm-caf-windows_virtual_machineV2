locals {
  ...
  serverType_3                  = substr(var.windows_VM.serverType, 0, 3)
  userDefinedString_7           = substr(var.userDefinedString, 0, 7)
  vm-name                       = replace("${local.env_4}${local.serverType_3}-${local.userDefinedString_7}", local.windows_virtual_machine_regex, "")
}